...
票 #293：T20 契約依據量錯。發生什麼事：拍 2^-20 時說「用掉 23.5%」，真實全雙精度是 5.43e-7、用掉 57%（舊量測用單精度阻抗）。結果：維持 2^-20 餘裕 1.76 倍；放寬 2^-19 用掉 28%。我建議：B 放寬（差是上一代單精度誤差，寫錯都在 1e-3 以上）。你回：A 維持／B 放寬。回完才合 #292、再開刀五報表的合併請求。

## 座標（給下一個對話）

現在在哪（2026-09-15 清晨，第九段四刀已合、兩刀等拍板）：
- 2026-09-09 清空重來，規矩從 `v2-audit/` 重新長；卡、票、考卷的數字全部看狀態頁（機器現算），這裡不抄。
- 引擎走到：鏡像法一到五段加第十段、第七段有限元素 v3（gmsh 網格＋scikit-fem P2＋pydiso 雙精度）在主線；第八段晚期混響 v3（每面 6×6 格、雙精度直接解）在主線；第九段（#280）決策紙 #288、刀一頻率軸與有限元素 300 Hz #289、刀三交接權重 #290、刀四幾何細軸 #291 在主線。
...
每一條標動詞，看了就知道要不要動：
- **等拍板再合**：PR #292（T20 上一代答案＋v3＋契約、T30 性質考卷，雲端綠，工作樹 `~/ghq/aosr-v3-s9-decay`）；分支 `feat/280-report`（刀五三路接合報表，疊在 #292 上，本機全綠、已推、工作樹 `~/ghq/aosr-v3-s9-report`），#292 合後重接主線、調地板再開 PR，關 #280。
- **第九段之後還要一題一題問**：①實測材料內插的量（阻抗或吸音率）、刻度、範圍外報錯，連帶只有頻帶值的材料靠「長度 6」辨認的風險；②256 階衰減掉不到 −35 dB 時 T20／T30 照回值要不要報錯；③報表命令列進寫法警衛放行名單；④報表 T20／T30 取帶中心頻率（助理補定）；⑤驗收席位 DeepSeek 斷線、改 Claude Opus 唯讀代班可不可以（#245）；⑥ρc 與密度×聲速不一致時報表照收；⑦完整報表有限元素 94 點單緒約 5 分鐘，進候選時間預算那題。
- **記下的坑**：引擎考卷在預設平行 `-n auto` 下 numpy 多緒互搶，報表考卷單跑 17 秒、平行時單題 45 秒以上；工人沙箱只寫 cwd 與 /tmp（指示別叫它寫 `~/aosr-v3-work/`）；程式裡寫上一代出處用模組點記法（`lib/...` 斜線路徑會被 refs 卡咬，別加放行名單）。

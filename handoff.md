...
- **停的位置是主線**，沒有在飛的分支；今天開的工作樹都刪了，更早留下的六棵還在（`git worktree list`：134、134-loaders、148-149、170、170-counterexample、room-pivot），其中 `~/ghq/aosr-v3-134-loaders` 是 PR #173 的草稿，別亂刪。引擎走到：鏡像法一到五段（直達到三階、振幅、總量）加第十段（多接收點、分格材料）都在主線；第六段 FEM 與第八段 ART 的參考答案入庫、契約拍了，v3 還沒寫。

...
現在在哪（2026-09-15 上午，第九段三路接合與物理量報表全部落地）：
...
每一條標動詞，看了就知道要不要動：
- **下一步可以開工（老闆已拍）**：#298 型錄頻帶吸音率當無規入射、Paris 公式反推實數阻抗（Z ≥ ρc）、α 在對數頻率直線內插到細軸再逐點反推、範圍外報錯、材料帶頻率清單（連 #295 一起關）；駐波管資料之後直接給阻抗。
- **候選卡（老闆說要開，照 `card-admission-threshold.md` 判）**：#296 基礎物理量單一來源（src 與考卷不准手寫聲速、密度、ρc，答案檔頭要等於設定檔）；#297 修完沒再驗就合（合併請求內文驗收結論要指到 head 那顆提交；機器只擋得住形狀、擋不住謊報）。
- **驗收席位**：DeepSeek 夜裡斷線、Claude Opus 唯讀代班，老闆同意並要備註（#245）；早上已恢復，換回。**修正後一定再送驗收，驗的就是要合的那顆 head**（夜裡省略過三次，#245 記）。
- **記下的坑**：引擎考卷在預設平行 `-n auto` 下 numpy 多緒互搶，報表考卷單跑約 17 秒、平行時單題 45 秒以上；工人沙箱只寫 cwd 與 /tmp（指示別叫它寫 `~/aosr-v3-work/`）；程式裡寫上一代出處用模組點記法（`lib/...` 斜線路徑會被 refs 卡咬，別加放行名單）；Claude Code 背景指令會被「低記憶體」誤判砍掉（它看 MemFree 不看 MemAvailable，官方 issue 92228／92448 未修），長計算用 setsid nohup＋結束標記檔＋監看。
- **報表還要記的事**：只收頻率無關實數阻抗；吸音很少的房（例如六面 α≈0.02）T20／T30 標算不出（#299）；報表驗收兩條非必修建議未做（模組 docstring 一句、原因字串可加逐點最低 dB）；有限元素與幾何在交接段的能量對齊沒有機器在守的檢查，只量過一次（見「交接段量過一次」）。
...
- 先不做：外部模擬器交叉驗證、脈衝響應；GUI 前端第九段報表已有，可以開題問老闆。

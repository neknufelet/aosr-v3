...
**晚期混響能量（ART）的精度契約取哪一種？** 票 #216。
- 發生什麼事：第八段第 1 段量完——凍結的上一代對參考房算晚期混響能量，三組材料六個頻帶，主對話重跑逐位相同、找碴兩輪。單精度對雙精度差百萬分之一；上一代的截尾級數對精確解差萬分之一。
- 結果：我們第四、五段的兩組材料平均吸收 0.64／0.68，在上一代自己標的域外（上限 0.5，偏差可達兩成）；加了一組域內對照。候選 A 照抄截尾級數、契約 2^-18（實測用到 26%）；B v3 精確解、契約 2·1e-4（用到 49%）。
- 我建議：A。鬆尺藏 bug；截尾規則三行算術，照抄成本低；要更對的精確解之後另立決策。
- 你回什麼：A 或 B。（還有三題同形狀排隊：#220 FEM 契約、#222 第五段直達能量常數要重拍、#223 立卡判準三的機器形式；回完這題再看。）
...
每一條標動詞，看了就知道要不要動：
- 要老闆回：#216 ART 契約（第一節）；#220 FEM 契約（建議 A 兩層）；#222 直達能量常數 2^-23 在別的接收點被超出、改 2^-20（建議 A）；#223 立卡判準三的機器形式（建議 A1：守卡的卡不參與、只對新候選跑）。
...
- **進行中**（2026-09-13 凌晨自動跑到 04:2x，段序見決策紙 `roadmap-stages-six-to-ten`）：第六段 #213、第八段 #215 證據量完（各三份、找碴過、主對話重跑過）停在契約票 #220、#216；第十段 #219 參考答案入庫分支 `feat/219-multi-receiver` 做好、找碴補過，只剩直達能量兩題紅等 #222；#217 判準三、四分支 `gov/217-admission-criteria-3-4` 第八回合全綠、第七回合等 #223 拍完重做（找碴摘要在 `~/aosr-v3-work/217/evidence/`）。工作樹 `~/ghq/aosr-v3-219`、`~/ghq/aosr-v3-217` 留著。

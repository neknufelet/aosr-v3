...
- 你回什麼：A 或 B。（另一題 FEM 的契約在票 #220，同一種形狀，回完這題再看。）
...
每一條標動詞，看了就知道要不要動：
- 要老闆回：#216 ART 契約（第一節）；#220 FEM 契約（兩層：flat 相容契約 2^-12 加剛性 20 Hz 以下對解析解 2^-10，建議 A）。
...
- **進行中**（2026-09-13 凌晨自動跑，段序見決策紙 `roadmap-stages-six-to-ten`）：第六段 #213 與第八段 #215 的第 1 段證據都量完、找碴過、主對話重跑過，各停在契約票（#220、#216，停車規則②）；證據在 `~/aosr-v3-work/213/`、`~/aosr-v3-work/215/`。第十段 #219 第 1 段參考答案（三接收點、y0 分格 2×2）量完、重跑逐位相同（`~/aosr-v3-work/219/`），多接收點沿用既有契約、v3 實作派工中。新開的票：#217 立卡判準三、四還沒變成機器；#218 頻率軸拉成設定。
- **已做完（今晚）**：第五段 #206 合了（總壓力與能量、命令列印總量）。

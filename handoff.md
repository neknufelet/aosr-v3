...
**精度契約：v3 算出來的距離與到達時間，要跟參考答案對到哪一位？** 票 #178。
- 發生什麼事：第一段做完了——用凍結的上一代跑出一間長方形房的直達音與六面一次反射，獨立用標準庫再算一次，七條逐位元相同；但換一種同樣正確的寫法會差最後一位。
- 結果：決策紙規定要在寫候選實作之前先訂精度契約，不准看到差異才放寬，所以現在要拍板。
- 我建議：A，逐位元相同（hex 字串相等），只管這一階段的幾何量；等不了的時候另立決策，不調數字。B 是容許最後幾位的差，代價是從第一天就有容差在。
- 你回什麼：A 或 B。
...
- **下一個動作**：等 #178 拍板精度契約，把它寫成決策紙、寫進考卷；然後開第二段的票。
...
- 再下一段（形狀已訂）：v3 給得出可改的輸入 → 跑一次 → 看得到路徑與到達時間（先用命令列印表，不做前端）→ 跟參考答案比。今天沒有任何一段程式做得到。

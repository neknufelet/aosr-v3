...
每一條標動詞，看了就知道要不要動：
- 要老闆回：沒有。
- **下一個動作**：參考答案那一段——用凍結的上一代（記號 `v3-donor`，打在上一代 aosr-v2 那個 repo 上、那個 repo 已整個設成唯讀）跑出「一間長方形房、一個音源、一個接收點、直達音加一階反射」，把輸入、單位、座標、材料假設、頻率與聲速與時間的慣例、版本、指令輸出、檔案雜湊全記下來，再獨立手算對一次幾何與到達時間。**這一段不寫產品程式**，交付的是證據。工作票 #175；轉向決策票 #174。

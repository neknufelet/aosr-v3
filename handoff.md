...
- **治理層盤點（2026-09-15 深夜到 09-16，票 #312 #313 #315 #296 #297）已落地，動手前要知道的五件**：①精度契約的尺（門檻數字）只住 `blueprint/precision_contracts.toml`，產品程式不准再寫一份、改值要帶新紙、每條一支變異考卷；②聲速、空氣密度這類基礎物理量只住 `src/aosr/config/data/physics_constants.toml`；③能力表 `src/aosr/config/data/capabilities.toml` 記每個入口驗過哪些房型、材料形式、頻率範圍、輸出欄，前端與之後的助理只照它開選項，表上 validated 要指到真的考卷；④被取代的決策紙要在同一支合併請求搬到 `docs/archive/`；⑤每支合併請求內文要一行「驗收：通過 <完整 head 提交 id>」，跟上主線後 head 變了要重驗。
...
每一條標動詞，看了就知道要不要動：
- **下一步可以開工（老闆已拍）**：#316 報表最小輸入／輸出契約（只在命令列邊界加正式模型、每欄記物理量／單位／基準／有效狀態、匯出 JSON schema，不立卡；schema 帶單位不會自動抓到用錯單位，單位靠對照考卷守）；物理那條線下一步 #303 幾何路能量收支（順序記在票上）。
- **記下沒卡會紅的洞（有票，不急）**：#318 型別警衛暫存目錄不可用時離開碼 1 而不是 2；#322 受管答案檔同一支合併請求改名成非 .json 會脫管；#326 雲端設定檔同一步重複鍵或沒有 run 沒有卡會紅。
...
- **報表還要記的事**：只收頻率無關實數阻抗（#309，能力表上已標 unsupported）；吸音很少的房（例如六面 α≈0.02）T20／T30 標算不出（#299）；報表驗收兩條非必修建議未做（模組 docstring 一句、原因字串可加逐點最低 dB）；有限元素與幾何在交接段的能量對齊沒有機器在守的檢查，只量過一次（見「交接段量過一次」）。
...
- **機器驗證的缺口（重判後再立候選票）**：輸入檔與答案檔正式 schema（#316 只做報表那一半）；物理性質的獨立真值只做了晚期混響那幾項（#323），整牆與同阻抗分格還沒有；時紅時綠偵測。

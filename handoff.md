...
- **治理修補在 repo 外的工作樹進行**，實際分支與保留工作樹以 `git worktree list` 為準；已合併的 228、234、235 三棵已移除，較早留下的六棵仍需保留（134、134-loaders、148-149、170、170-counterexample、room-pivot），其中 `~/ghq/aosr-v3-134-loaders` 是 PR #173 的草稿，別亂刪。引擎走到：鏡像法一到五段（直達到三階、振幅、總量）加第十段（多接收點、分格材料）都在主線；第六段 FEM 與第八段 ART 的參考答案入庫、契約拍了，v3 還沒寫。
- **PR #173（分支 `feat/134-materials-loaders`）已轉草稿、還沒合、尚未通過驗收**：雲端 `verify` 紅在三個插值浮點數比對案例。**原因還沒定案**——不准把它寫成某個猜測，要動先實跑；程式與證據保留，不當成已完成成果。
- **待處置 PR #168**：仍開著、不是草稿；舊的雲端 `verify` 曾綠，但現在與主線衝突，不能把舊綠當成可合併。要恢復就先處理衝突、按現行規矩重跑，再決定保留或由新 PR 取代。
...
- **重判後再立候選票的三題（目前沒有候選票號）**：①輸入檔與答案檔是否還缺一份正式 schema；②互易、能量非負、整牆與同阻抗分格等物理性質還缺哪些獨立真值；③時紅時綠的偵測還缺什麼。不能照舊描述直接立三卡：輸入驗證與物理性質已有部分考卷，雲端 `verify` 的 15 分鐘上限也已由現有卡守著，相關待決事項另在 #120；先按立卡四條判準逐題量現有覆蓋與唯一必紅原因。
...
- 做法：PO 正在試用新的監督／施工／獨立驗收組合，精確模型、用力程度與「兩次未過後只複驗一次，再停下等裁決」的規則記在 #245；這是試驗，不是永久專案規則。外部模型走 calling-other-models（派工工具）並留收據，主對話逐條重跑、終審與合併；實作一律在**獨立工作樹**（`git worktree add` 到 repo **外面**，別放 `.claude/` 底下免得髒了主樹）做；派工指示寫成檔案、只給路徑；PR 內文用 `Closes #n` 關票，不准手動關。獨立驗收固定唯讀，沒有收據或無法重現的結論不採信。

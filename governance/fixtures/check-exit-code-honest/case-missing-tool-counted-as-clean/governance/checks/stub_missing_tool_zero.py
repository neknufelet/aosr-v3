#!/usr/bin/env python3
"""樣本用的假檢查：git 不在 PATH 就跳過檢查回 0。

壞在：外部工具缺席時它「跳過」並回 0。工具缺席等於這一跑沒量到東西，只能回 2。
...
def main() -> int:
    ...
    args = ap.parse_args()
    if shutil.which(ENUMERATE_ARGV[0]) is None:
        report(args.scan_root, 0, 0)
        print("WARN git 不在 PATH，跳過檢查", file=sys.stderr)
        return 0

#!/usr/bin/env python3
"""樣本用的假檢查：把列舉子程序的退出碼布林化，非零就當成「有違規」。

壞在：`rc = proc.returncode` 之後寫 `if rc:`——1（有違規）與 128（工具自壞）都是真值，兩種結局被摺成一種，然後回 1。
...
def main() -> int:
    ...
    try:
        ...
        report(args.scan_root, 0, 0)
        print("FAIL(2) 列舉用的外部工具不在 PATH", file=sys.stderr)
        return 2
    rc = proc.returncode
    if rc:
        report(args.scan_root, 0, 1)
        print("FAIL(1) 列舉失敗，當成有違規", file=sys.stderr)
        return 1
    names = [ln for ln in proc.stdout.splitlines() if ln.strip()]
    if not names:
        report(args.scan_root, 0, 0)

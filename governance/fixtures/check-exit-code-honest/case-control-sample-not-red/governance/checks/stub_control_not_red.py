#!/usr/bin/env python3
"""樣本用的假檢查：尺寫壞了，已知會咬的控制樣本餵下去它回 0。

壞在：它找的是 NEVER-BITES 這個檔名，控制樣本裡放的是 BITE，所以它在「已知有一筆違規」的輸入上回 0——v2 lightrag「乾淨的 0」的形狀。
...
def main() -> int:
    ...
    try:
        ...
        return 2
    hits = [n for n in names if Path(n).name == "NEVER-BITES"]

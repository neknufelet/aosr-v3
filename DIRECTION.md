...
| `src/` | 產品程式（引擎）的新家，套件名 `aosr`。**今天是空的**——v2 的舊引擎不搬進來，一塊（一個子套件）一張票、一塊一塊重新長，帶那一塊的測試與標準答案，從第一塊起就被現有的規矩全咬（決策紙 `docs/decisions/engine-not-imported-new-home-grows-block-by-block.md`） |

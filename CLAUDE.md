...
- **layers-import-downward-only**（擋合併）：新家的 import 只准往下流：`src/aosr/` 底下每一支 .py 的每一個 import（含寫在函式裡的延遲 import，AST 走全樹、不只看檔頭）都要對回它自己那一層，下層 import 上層即紅、模組之間成環即紅、模組沒登記在層次表裡即紅。

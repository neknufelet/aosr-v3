...
- **decision-paper-structure**（擋合併）：決策紙一題一檔，格式與取代關係由機器守。七條：①frontmatter 八格必填（題目、建檔日、改檔日、狀態、種類、取代誰、被誰取代、摘要），少一格就紅（取代那兩格准填空字串，其餘空的等於沒填）；②正文段落必須有問題、選項、決定、為什麼、代價，加上拍板，標題正規化後逐字相等，寫成「問題與背景」不算；③status 只認 accepted、superseded、proposed，自創的狀態等於沒有狀態；④標了 superseded 就必須有 superseded_by，而且指到的檔要真的在；⑤取代關係雙向對稱——某張說取代誰，被取代那張就要指回來，反過來也一樣；⑥沿 superseded_by 走不得成環；⑦同一題只准一份 accepted，而「同題」用機器判得出來的鍵認：連在同一條取代鏈上的檔算同一題（靠標題判同一題機器判不出來，所以只認鏈）。取代關係的值寫成同一層裡那份決策紙的檔名（不帶目錄）。日期只比 frontmatter 那兩格，不比 git 的提交日——squash merge 之下作者寫檔當天不可能預知合併日。
- **doc-frontmatter-and-dates**（擋合併）：docs 底下的設計文件與知識文件要有齊全的標頭，三類的份數各有上限，三類裡面不准再分層。三條：①`docs/design` 與 `docs/cairn` 的每一份 .md，frontmatter 必填題目、建檔日、改檔日、種類、摘要，少一格或留空就紅；種類那一格的值必須跟它住的那個目錄同名（住 design 就要寫 design、住 cairn 就要寫 cairn），改檔日不准早於建檔日，日期讀不成日期也紅。**刻意不比 git 的提交日**——squash merge 之下作者寫檔那天不可能預知合併日，實測過，拿提交日當尺會變成作者滿足不了的要求。②三類各有份數上限：拿 `git ls-files` 的列舉集合逐類數 .md（以版控索引為準，不走檔案系統掃描——v2 就是用 rglob 把被忽略的檔算進去假紅），超過那一類登記在這張卡 `[settings]` 的上限就紅；`.gitkeep` 這種不是文件的檔不算。上限是初值，改要走 PR。③三類裡面不准再開子目錄：docs 底下的檔只准住在「類別目錄／檔名」那一層，再深一層就紅——決策紙寫的是「平行不分層」，而 docs 根層的目錄白名單只看得到最上面那一層。分工：決策紙的欄位與取代鏈歸 decision-paper-structure（這張卡只數它的份數、不看它的欄位）；docs 根層准出現什麼歸 file-placement-allowlist（這張卡補它看不到的深度）；單檔多大歸 doc-size-cap（這張卡管每一類有幾份）。刻意沒管：決策紙第三道柵欄「算得出來的不存檔」——一份手抄的架構圖跟一份真的設計文件長得一模一樣，機器判不出來，這一格靠人看 PR。讀不到這張卡的 `[settings]`、docs 那一層一個檔都沒有、某份 md 讀不開，一律回 2 不回 0。
...
- **no-model-names-in-entry-files**（擋合併）：每次開工都會載入的檔——兩份入口檔，加上 `docs/decisions/` 底下每一份決策紙——**全文**不准出現卡上字典列的那些字：`model_words`（各家模型的名字）、`cli_words`（模型家那幾支 CLI 的名字）、`quota_words`（講「用得完／用不完」那類字樣）三張清單，比對不分大小寫、只咬整個詞——前後不准是英文字母或底線，所以名字後面接版本數字一樣咬，`.` 與 `-` 兩邊算斷開（名字被寫進網址、檔名、複合詞裡照樣咬）。字典是這張卡 `[settings]` 的三張清單，那是它唯一的家：檢查程式沒有預設值，讀不到就回 2 不回 0；派工工具那份清單今天不在這個 repo 裡，等它進得了版控要改成從它生成，不是兩邊各抄一份。放行只有一種寫法——`[[settings.allow]]`，一條一段字面字串，逐字（連大小寫）相同的那一段先被塗掉再比對，每條要寫理由與到期日；今天兩條，都是結構上躲不掉的字：入口檔自己的檔名，以及對話網址的開頭（規格說「路由只存在於派工工具」，網址不是路由）。放行只塗掉那一段字，同一行別的地方出現同一個名字照樣咬。掃描面上一個檔都沒有、某一份檔不是合法 UTF-8、讀不到 `[settings]`、宣告這支檢查的卡不是剛好一張，一律回 2。為什麼連決策紙也掃：v2 的模型名不只漂進入口檔，也漂進了每次開工都會讀的決策索引，一換就過期說謊。

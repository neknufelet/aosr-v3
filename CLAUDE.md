...
- **precision-contracts-live-in-one-registry**（擋合併）：精度契約的尺只住一份登記簿 `blueprint/precision_contracts.toml`，三顆牙：①產品程式 `src/` 底下每一支 .py，模組載入時就算好的數字（模組層級常數、類別身體、函式簽章預設值，藏在模組層的 if／try 底下一樣算），名字命中卡上登記的樣式（`*_CONTRACT_*` 這類）即紅——判形狀不判數值，抓的是「第二個家」；②本次提交範圍裡登記簿有哪幾條的值或人看寫法變了（含新增），每一條指名的決策紙必須是同範圍新增的檔、住在登記的決策目錄之一、內文同時出現那條的名字與新寫法、status 是 accepted——一份無關的紙不算帶紙；③登記簿每一條的 decision_paper 在決策目錄或封存區找得到，mutant_test 指的考卷檔在版控裡而且真的定義了那個測試函式。
...
- **type-guard**（擋合併）：型別警衛，掃版控裡的每一支 `.py`（Python 程式檔）與 `.pyi`（型別存根檔）（扣掉必紅樣本樹與本機工具目錄），兩層，第二層刻意獨立於 `mypy`（型別檢查器）。
